...
const PLCSimulator = () => {
  ...
  const [submissionType, setSubmissionType] = useState('save'); // 'save' | 'copy'
  ...
  // --- SAVE / COPY LOGIC ---
  const handleSubmissionRequest = (type) => {
  ...
  const executeSubmission = (e) => {
    e.preventDefault();
    if (!studentInfo.name || !studentInfo.id || !studentInfo.section) {
    ...
    const payload = {
      ...
      classSection: studentInfo.section,
      plantKey: selectedPlant,
      plantTitle: PLANT_TEMPLATES[selectedPlant].name_en,
      ioMap,
      rungs,
    ...
    if (submissionType === 'save') {
      ...
      a.download = `PLC_Submission_${studentInfo.id}_${selectedPlant}_${dateStr}.json`;
      a.click();
      setShowSubmissionModal(false);
    } else {
      // Copy to clipboard (WeChat fallback)
      ...
      const copyToClipboard = () => {
        ...
        try {
            ...
            alert("Submission code copied. Paste it into WeChat and send it to your lecturer.");
        } catch (err) {
            alert("Failed to copy. Please try again on a PC.");
        }
    ...
    reader.onload = (event) => {
      try {
        ...
        if ((data.plant || data.plantKey) && data.rungs && data.ioMap) {
          setIsRunning(false);
          setSelectedPlant(data.plantKey || data.plant);
          setIoMap(data.ioMap);
          setRungs(data.rungs);
          ...
          <div className="max-w-5xl mx-auto py-12 px-8 relative flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.entries(PLANT_TEMPLATES).map(([key, template]) => (
                <div key={key} onClick={() => loadTemplate(key, 'student')} className="bg-white rounded-2xl p-6 border-2 border-slate-200 hover:border-blue-500 hover:shadow-xl transition-all cursor-pointer group flex flex-col h-full">
